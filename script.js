...
if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                formStatus.innerHTML = '<span style="color: #00ff88;">Message sent successfully! <i class="fa-solid fa-check"></i></span>';
                contactForm.reset();
            } else {
                const data = await response.json();
                if (Object.hasOwn(data, 'errors')) {
                    formStatus.innerHTML = '<span style="color: #ef4444;">' + data["errors"].map(error => error["message"]).join(", ") + '</span>';
                } else {
                    formStatus.innerHTML = '<span style="color: #ef4444;">Oops! There was a problem.</span>';
                }
            }
        } catch (error) {
            formStatus.innerHTML = '<span style="color: #ef4444;">Submission failed. Please check your connection.</span>';
        } finally {
